...
<main>
    ...
    <div id="view">
        ...
        <div><label class="drgup"l>본문</label>
            <span id="contents">${board.contents}</span></div>

        <label></label>
            <button type="button" id="newbdbtn" class="btn btn-success" >새글쓰기</button>
            <button type="button" class="btn btn-light" >목록으로</button>
        <c:if test="${(not empty sessionScope.member) && (sessionScope.member.userid eq board.userid)}">
